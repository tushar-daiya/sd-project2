...
async function fetchFunnySearchResults(query: string, mode: string) {
  ...
  if (mode === "work") {
    prompt =
      "Reply with a quirky, funny, and casual response to the following query: " +
      query +
    ...
    prompt =
      "Reply with a super serious, strict, and authoritative scolding response to the following query:" +
      query +
    ...
    prompt =
      "Reply with a hilarious roast to the following query:" +
      query +
...
export default async function page(props: {
  ...
  return (
    <div className="w-full">
      ...
      <div className="py-20 mx-auto max-w-3xl">
        {data2 && <p className="text-lg">{data2}</p>}
        {data1 && <p className="text-lg mt-8">{data1}</p>}
      </div>
    </div>
  );
}
